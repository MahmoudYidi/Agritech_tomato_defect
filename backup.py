...
#hsi = sp.open_image('/workspace/src/dataset/hsi/anom_6_2024-01-26_16-25-40/capture/anom_6_2024-01-26_16-25-40.hdr')
#hsi= sp.open_image('/workspace/src/test/tomato_tomato_demo_2024-08-12_13-47-42/capture/tomato_tomato_demo_2024-08-12_13-47-42.hdr') #Norm
# Load the white and dark references
white_ref = sp.open_image("/workspace/band_selection/white_ref_2024-01-26_16-00-30/capture/white_ref_2024-01-26_16-00-30.hdr")
dark_ref = sp.open_image("/workspace/band_selection/dark_ref_shutter_cap_on/capture/dark_ref_shutter_2024-01-26_16-03-56.hdr")

# white_ref = sp.open_image("/workspace/src/test/white_ref/capture/tomato_tomato_demo_2024-08-12_11-59-43.hdr")
# dark_ref = sp.open_image("/workspace/src/test/dark_ref/capture/tomato_tomato_demo_2024-08-12_12-05-08.hdr")
...
plt.plot(wavelengths,mean_spectrum_split, label='Split Region')
...
smoothed_split = savgol_filter(mean_spectrum_split, window_length=9, polyorder=2)
smoothed_normal = savgol_filter(mean_spectrum_normal, window_length=9, polyorder=2)
plt.plot(wavelengths, smoothed_split, label='Split Region')
plt.plot(wavelengths, smoothed_normal, label='Normal Region')
plt.xlabel('Band Number')
plt.ylabel('Reflectance')
plt.title('Mean Spectrum of Split vs Normal Regions')
plt.legend()
plt.show()
#split_first_derivative = np.gradient(smoothed_split)
#normal_first_derivative = np.gradient(smoothed_normal)
# plt.plot(split_first_derivative , label='Split Region')
# plt.plot(normal_first_derivative, label='Normal Region')
# plt.xlabel('Band Number')
# plt.ylabel('Reflectance')
# plt.title('Mean Spectrum of Split vs Normal Regions')
# plt.legend()
...
# split_second_derivative = np.gradient(split_first_derivative)
# normal_second_derivative = np.gradient(normal_first_derivative)
# plt.plot(split_second_derivative, label='Split Region')
# plt.plot(normal_second_derivative, label='Normal Region')
# plt.xlabel('Band Number')
# plt.ylabel('Reflectance')
# plt.title('Mean Spectrum of Split vs Normal Regions')
# plt.legend()
...
# Statistically. 
band_differences = np.abs(smoothed_split - smoothed_normal)
#band_differences = np.abs(split_first_derivative - normal_first_derivative)
N = 15 # Number of bands to select
selected_bands = np.argsort(band_differences)[-N:]
#selected_bands2 = np.argsort(band_differences2)[-N:]

print(f"Selected Bands: {selected_bands}")
print("Band Index | Wavelength (nm)")
print("-----------------------------")
for band_index in selected_bands:
    wavelength = wavelengths[band_index]
    print(f"{band_index:10} | {wavelength:10.2f}")
#print(f"Selected Bands: {selected_bands2}")
...
selected_data = hsi_normalized[:, :, selected_bands]  # Extract the selected bands
combined_data = np.mean(selected_data, axis=2) 
height, width, num_bands = selected_data.shape

from sklearn.cluster import KMeans
height, width, num_bands = selected_data .shape
# Reshape the selected data for clustering
X = selected_data .reshape(-1, num_bands)
...
# correlation_matrix = np.corrcoef(selected_data.reshape(-1, num_bands).T)

# # Select bands with low correlation
# N_final = 10  # Number of final bands to select
# selected_final_bands = np.argsort(np.sum(correlation_matrix, axis=1))[:N_final]
# print(f"Final Selected Bands: {selected_final_bands}")
# final_data = selected_data[:, :, selected_final_bands]

# ########################################################################

# normal_spectrum = final_data[y1_n:y2_n, x1_n:x2_n, :]
# anomalous_spectrum = final_data[y1_a:y2_a, x1_a:x2_a, :]

# # Compute the mean spectrum for each region
# mean_normal_spectrum = np.mean(normal_spectrum, axis=(0, 1))
# mean_anomalous_spectrum = np.mean(anomalous_spectrum, axis=(0, 1))

# # Plot the mean spectra
# plt.figure(figsize=(8, 5))
# plt.plot(selected_final_bands, mean_normal_spectrum, label='Normal Region', color='green')
# plt.plot(selected_final_bands, mean_anomalous_spectrum, label='Anomalous Region', color='red')
...
# from sklearn.cluster import KMeans
# height, width, num_bands = final_data.shape
# # Reshape the selected data for clustering
# X = final_data.reshape(-1, num_bands)
...
#############################################################################################
#height, width, num_bands = selected_data.shape
#selected_data_2d = selected_data.reshape(-1, num_bands)  # Reshape to (height * width, num_bands)

# Step 3: Perform PCA
#pca = PCA(n_components=num_bands)  # Keep all components
#pca_result = pca.fit_transform(selected_data_2d)  # Transform the data

# Step 4: Reshape the PCA result back to image dimensions
#pca_image = pca_result.reshape(height, width, num_bands)  # Shape: (height, width, num_bands)

# Step 5: Display the principal components side by side
#fig, axes = plt.subplots(1, num_bands, figsize=(15, 5))

#for i in range(num_bands):
  #  ax = axes[i]
  #  im = ax.imshow(pca_image[:, :, i], cmap='gray')  # Display each PC
...
N = 20 # Number of bands to select
selected_bands = np.argsort(band_differences)[-N:]
#selected_bands = np.argsort(band_differences)[:N]
